...
### 6.1 `parse_document_with_mineru`
...
- 若 `run_standard_review` 收到 PDF 或 Word 而没有 Markdown，应返回“需要先调用 MinerU 解析”的结构化提示，或由 reviewer subagent 在调用审核前主动调用 `parse_document_with_mineru`。
- 若当前代码已存在 `parse_pdf_with_mineru`，建议将其收敛为 `parse_document_with_mineru` 的 PDF 分支，或保留为兼容别名；对 Agent 暴露的首选入口应是统一的 `parse_document_with_mineru`。
...
- PDF 或 Word 缺少 Markdown 时，先调用 parse_document_with_mineru。
- 不手工拼接审核流程。
- 不伪造标准条款和来源。
- 不覆盖原始文件。
...
1. 确认输入文件和解析产物。
2. 若只有 PDF 或 Word，调用 parse_document_with_mineru。
...
- `parse_document_with_mineru` 是独立 tool，负责把 PDF 或 Word 转为 Markdown，并生成图片、布局结果、manifest 和 MinerU trace。
